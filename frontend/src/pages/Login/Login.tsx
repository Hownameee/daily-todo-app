	...
	return (
		<div className="flex justify-center items-center w-full h-full p-4">
			<div className="w-full max-w-md p-8 bg-slate-900/60 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 relative overflow-hidden ring-1 ring-white/5 animate-fade-in-up">
				<div className="absolute top-0 left-0 w-full h-0.5 bg-linear-to-r from-transparent via-cyan-400 to-transparent opacity-70"></div>

				<h2 className="text-3xl font-bold text-white text-center mb-6 tracking-wide drop-shadow-md">Login</h2>

				{!(errorMessage === "") && <div className="p-3 mb-5 text-center text-red-200 bg-red-900/40 border border-red-500/30 rounded-lg text-sm backdrop-blur-sm shadow-inner">{errorMessage}</div>}
				...
				<form onSubmit={handleSubmit(handleLogin)} className="flex flex-col gap-5">
					<div>
						...
						<input
							...
							id="username"
							required
							className={`w-full p-3.5 rounded-xl bg-black/40 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 border ${errors.username ? "border-red-500/50" : "border-slate-700/50 hover:border-slate-600"} focus:border-cyan-500 transition-all duration-200 shadow-inner`}
					...
					<SubmitButtonFormData
						...
						value="Login"
						className="w-full py-3.5 mt-4 rounded-xl bg-linear-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold shadow-lg shadow-cyan-900/20 hover:shadow-cyan-500/40 transition-all duration-300 transform hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-cyan-400 cursor-pointer border border-white/10"
					/>
				</form>
